...
This paper focuses on \emph{backdoor attacks}, 
where some perturbations are added to the data (observations) such that given normal observations, the agent takes high-rewards actions, and low-reward actions on observations injected with \emph{triggers}.

In this paper, we propose \textsc{Baffle} (\textbf{B}ackdoor \textbf{A}ttack for O\textbf{ff}line Reinforcement \textbf{Le}arning), an approach that automatically implants backdoors to RL agents by poisoning the offline RL dataset, and evaluate how different offline RL algorithms react to this attack.
